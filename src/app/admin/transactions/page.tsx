...
export default function AdminTransactionsPage() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Fetch user display names for unique userIds
        const ids = Array.from(
          new Set(rows.map((r) => r.userId).filter(Boolean))
        ) as string[];
        const missing = ids.filter((uid) => !(uid in userNameMap));
        if (!cancelled && db && missing.length > 0) {
            ...
            missing.map(async (uid) => {
              try {
                ...
                if (!data) {
                  ...
                  if (qSnap.empty) {
                    const anyRow = rows.find((r) => r.userId === uid);
                    const email =
                      anyRow?.email || (anyRow as any)?.Email || null;
                    if (email) {
                      q = query(
                        usersCol,
                      ...
                      if (qSnap.empty) {
                        q = query(usersCol, where("Email", "==", email));
                        qSnap = await getDocs(q);
                      }
                    }
                  }
                  if (!data && qSnap && !qSnap.empty) {
  ...
  const formatDate = (ts?: Timestamp) =>
    ts
      ? new Date((ts as any).toMillis?.() ?? ts.seconds * 1000).toLocaleString()
      : "—";
  const formatDateOnly = (ts?: Timestamp) =>
    ts
      ? new Date(
          (ts as any).toMillis?.() ?? ts.seconds * 1000
        ).toLocaleDateString()
      : "—";
  const formatAmount = (amt: number | undefined, cur: string | undefined) =>
    `${cur || ""} ${Number(amt || 0).toFixed(2)}`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            View and manage transactions and invoices.
          </p>
        </div>
                    ...
                    >
                      ...
                      <td className="py-3 pr-4">
                        <span className="inline-flex items-center gap-2">
                          <span className="px-2 py-1 rounded-full text-xs bg-sky-50 text-sky-700 ring-1 ring-sky-200">
                            {tx.packageKey || "pkg"}
                          </span>
