...
export default function AdminUKCompanyFormationPage() {
  ...
  const [packages, setPackages] = useState<UkPackage[]>([]);
  const [viewPkg, setViewPkg] = useState<UkPackage | null>(null);

  // realtime list
  useEffect(() => {
    const q = query(collection(db, "Uk packages"), orderBy("createdAt", "desc"));
    const unsub = onSnapshot(q, (snap) => {
      const list: UkPackage[] = snap.docs.map((d) => {
        const data = d.data() as Partial<UkPackage>;
        return {
          id: d.id,
  ...
  const handleDelete = async (p: UkPackage) => {
    if (!p.id) return;
    const ok = typeof window !== "undefined" ? window.confirm(`Delete service "${p.name}"?`) : true;
    if (!ok) return;
    try {
      await deleteDoc(doc(db, "Uk packages", p.id));
    } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsSaving(true);
      await addDoc(collection(db, "Uk packages"), {
        name: name.trim(),
        originalPrice: orig,
  ...
  return (
    <section className="py-10 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">UK Company Formation</h1>
          <p className="text-gray-600">Add and manage UK packages.</p>
        </div>
...
function EditForm({ pkg, onClose }: { pkg: UkPackage; onClose: () => void }) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await updateDoc(doc(db, "Uk packages", pkg.id), payload as Record<string, unknown>);
      setOk("Updated");
