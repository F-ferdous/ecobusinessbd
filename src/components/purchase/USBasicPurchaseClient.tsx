...
export default function USBasicPurchaseClient() {
  ...
  const PACKAGE_META: Record<string, { title: string; planPrice: number; stateFee: number; heading: string; note?: string; }> = {
    'basic': { title: 'Basic Package', planPrice: 149, stateFee: 199, heading: 'USA LLC Formation – Basic', note: 'One-time' },
    'standard': { title: 'Standard Package', planPrice: 349, stateFee: 449, heading: 'USA LLC Formation – Standard', note: 'One-time' },
    'premium': { title: 'Premium Package', planPrice: 449, stateFee: 579, heading: 'USA LLC Formation – Premium', note: 'One-time' },
    'amazon-plus': { title: 'Amazon Plus', planPrice: 449, stateFee: 749, heading: 'USA LLC Formation – Amazon Plus', note: 'One-time' },
  };
  ...
  const US_STATES = [
    'Delaware','Wyoming','Florida','Texas','New York','California','Nevada','New Mexico','Colorado','Georgia','Illinois','Washington','Arizona','North Carolina','Pennsylvania','Virginia','Utah','Oregon','Maryland'
  ];
  const COMPANY_TYPES = ['LLC','S-Corporation','C- Corporation','Partnership'];
  const SERVICE_TYPES = [
    'E-Commerce',
    'Health',
    'B2B Software',
  ...
  const handleProceedToCheckout = async () => {
    ...
    if (!state || !companyType || !serviceType || !memberType || !proposedName.trim()) {
    ...
    try {
      ...
      try {
        ...
        const orderDetails = {
          ...
          amount: total,
          currency: 'USD',
          company: { state, companyType, proposedName: proposedName.trim(), serviceType, memberType },
      ...
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          cancelPath: `/purchase/us/purchase?package=${encodeURIComponent(pkgKey)}`,
        }),
      });
      if (!res.ok) {
    ...
    <div className="bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-12">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2 text-base">Review your package, select extra services, apply coupon, and checkout.</p>
        </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-gray-600 text-sm mt-1">
                    Includes company formation, registered office address, and company documents.
                  </p>
                </div>
            ...
            <section className="bg-emerald-50 rounded-2xl shadow ring-1 ring-emerald-100 p-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select required value={state} onChange={(e) => setState(e.target.value)} className="w-full px-4 py-3 text-base border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500">
                    {US_STATES.map((s) => (
                ...
                <div className="md:col-span-2">
                  ...
                  <input required value={proposedName} onChange={(e) => setProposedName(e.target.value)} className="w-full px-4 py-3 text-base border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Enter proposed company name" />
                </div>
                <div>
